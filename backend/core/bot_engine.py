"""Bot Engine — v3 Fixed (auto-restart + Upstox live trading)"""
...
class BotEngine:
    ...
    async def start(self, symbol: str = None, capital: float = None, mode: str = "paper"):
        ...
        # Connect broker for live mode
        if mode == "live":
            ok = await connect_broker()
            if not ok and settings.HARD_STOP_ON_API_FAIL:
                logger.error("Broker connection failed — cannot start live mode")
                await add_notification(
                    "EMERGENCY", "Broker Failed",
                    "Cannot connect to broker. For Upstox: login via dashboard first."
                )
                return
        ...
        # PERSIST state to DB — critical for auto-restart after Render sleep
    ...
    async def stop(self):
        ...
        for t in [self._task, self._monitor_task, self._btst_task]:
            if t:
                t.cancel()
        # PERSIST stopped state — prevents auto-restart next time
        await set_config("bot_status", "stopped")
        await _broadcast("bot_status", {"running": False})
        logger.info("🔴 Bot STOPPED")

    async def _reload_config(self):
        """Load all config from DB — called on start and during signal loop."""
    ...
    async def _check_and_trade(self):
        ...
        if self.last_trade_date != today:
            ...
            self.trading_halted_today = False
            self.api_fail_count       = 0
            logger.info(f"📅 New day {today} — daily reset")
        ...
        if self.daily_pnl <= -daily_cap:
            ...
            msg = f"Daily loss cap hit (₹{self.daily_pnl:.0f}) — halting today"
    ...
    async def _enter_trade(self, signal: Dict):
        ...
        spot        = signal.get("spot_price", ltp * 100)
        ...
        sizing = calculate_adaptive_size(
            ...
            consecutive_losses=self.consecutive_losses, symbol=self.symbol,
        )
        qty      = sizing["quantity"]
        lots     = sizing["lots"]
        ...
        exec_result = await execute_order(
            symbol=self.symbol,       option_type=option["option_type"],
            strike=option["strike"],  expiry=option["expiry"],
            quantity=qty,             action="BUY",
            ltp=ltp,                  token=option.get("token", ""),
            mode=self.mode,
        )
        ...
        notif = (
            ...
            f"Fill ₹{fill_price} (Slip {exec_result['slippage_pct']:.1f}%) | "
            f"{lots}L | Score={signal['score']} | {exec_result['broker'].upper()}"
        )
        await add_notification("TRADE", "Trade Entered", notif)
        await _broadcast("trade_entered", {**trade, "id": trade_id, "sizing": sizing})
        logger.info(
            f"📈 ENTERED #{trade_id} | Fill=₹{fill_price} | "
            f"Slip={exec_result['slippage_pct']:.1f}% | {lots}L | {exec_result['broker'].upper()}"
        )
    ...
    async def _monitor_positions(self):
        price_data = await fetch_live_price(self.symbol)
        if not price_data:
            return
        ...
        for trade in list(self.open_trades):
            ...
            # Estimate option LTP from spot movement (paper) or fetch real LTP (live)
            if self.mode == "live" and settings.BROKER == "upstox":
                try:
                    from core.broker import get_ltp as upstox_ltp
                    trading_sym = f"{self.symbol}{trade['expiry']}{opt_type}{int(trade['strike'])}"
                    real_ltp = await upstox_ltp(f"NSE_FO|{trading_sym}")
                    est_ltp  = real_ltp if real_ltp else entry
                except Exception:
                    est_ltp = entry
            else:
                entry_spot = ((trade.get("signal") or {}).get("price_data") or {})
                entry_spot = entry_spot.get("price", spot) if isinstance(entry_spot, dict) else spot
                chg_pct    = (spot - entry_spot) / max(entry_spot, 1)
                delta      = 0.5
                est_ltp    = max(
                    round(entry * (1 + chg_pct / delta if opt_type == "CE" else 1 - chg_pct / delta), 2),
                    0.05
                )
            ...
            if tid not in self.partial_done and est_ltp >= partial_tgt:
                ...
                exec_r = await execute_order(
                    self.symbol, opt_type, trade["strike"], trade["expiry"],
                    half_qty, "SELL", partial_tgt, mode=self.mode,
                )
            ...
            # Trail SL (locks in 60% of profit)
            if est_ltp > entry:
            ...
            if exit_reason:
                exec_r = await execute_order(
                    self.symbol, opt_type, trade["strike"], trade["expiry"],
                    trade["quantity"], "SELL", est_ltp, mode=self.mode,
                )
                ...
                if pnl > 0:
                    ...
                    if self.consecutive_losses >= self.day_stop_losses:
                        self.trading_halted_today = True
                        msg = f"{self.day_stop_losses} consecutive losses — halted for today"
                ...
                icon = "🟢" if pnl > 0 else "🔴"
                logger.info(f"{icon} CLOSED #{tid} | {exit_reason} | ₹{pnl:+.0f} | {exec_r['broker'].upper()}")
                await add_notification(
                    "TRADE" if pnl > 0 else "WARNING",
                    f"Trade {exit_reason.replace('_', ' ')}",
                    f"#{tid} | P&L ₹{pnl:+.0f} | Fill ₹{fill}"
                )
    ...
    async def _check_btst(self):
        ...
        for bt in list(self.btst_trades):
            reason = await should_exit_btst(bt, spot)
            if reason:
                exec_r  = await execute_order(
                    self.symbol, bt["option_type"], bt["strike"], bt["expiry"],
                    bt["quantity"], "SELL", spot, mode=self.mode,
                )
                fill    = exec_r["fill_price"]
        ...
        qty    = max(settings.LOT_SIZE_NIFTY, int(risk_a / max(sl_pu, 0.01)))

        exec_r = await execute_order(
            self.symbol, opt["option_type"], opt["strike"], opt["expiry"],
            qty, "BUY", ltp, mode=self.mode,
        )
