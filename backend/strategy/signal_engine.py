...
async def generate_signal(
    ...
    # Developer synthesize flag `DEV_FORCE_SIGNAL` is disabled in production
    # to ensure only real market data drives signals. If set, it will be ignored.
    dev_force = os.getenv("DEV_FORCE_SIGNAL")
    if dev_force and dev_force.lower() in ("1", "true", "yes"):
        logger.warning("DEV_FORCE_SIGNAL is set but ignored — synthetic signals disabled.")
    if filters is None:
        filters = {}
