...
async def debug_upstox(endpoint: str, symbol: str = "NIFTY"):
    """
    Call Upstox API directly and return raw response for debugging.
    endpoint: 'contract' | 'chain' | 'ltp' | 'ohlcv' | 'profile'
    """
    import httpx
    from data.upstox_market import _get_token, _headers, INDEX_KEYS, UPSTOX_BASE, _enc
    from urllib.parse import quote

    try:
        token = await _get_token()
        ikey  = INDEX_KEYS.get(symbol.upper(), f"NSE_INDEX|{symbol}")
        headers = _headers(token)

        async with httpx.AsyncClient(timeout=15) as c:
            if endpoint == "profile":
                resp = await c.get(f"https://api.upstox.com/v2/user/profile", headers=headers)
            elif endpoint == "contract":
                resp = await c.get(f"{UPSTOX_BASE}/option/contract",
                    params={"instrument_key": ikey}, headers=headers)
            elif endpoint == "chain":
                from data.upstox_market import get_nearest_expiry
                exp = await get_nearest_expiry(symbol.upper()) or "2026-04-17"
                resp = await c.get(f"{UPSTOX_BASE}/option/chain",
                    params={"instrument_key": ikey, "expiry_date": exp}, headers=headers)
            elif endpoint == "ltp":
                resp = await c.get(f"{UPSTOX_BASE}/market-quote/ltp",
                    params={"instrument_key": ikey}, headers=headers)
            elif endpoint == "ohlcv":
                enc_key = _enc(ikey)
                url = f"{UPSTOX_BASE}/historical-candle/intraday/{enc_key}/5minute"
                resp = await c.get(url, headers=headers)
            else:
                return {"error": f"Unknown endpoint: {endpoint}"}

        body = resp.json()
        # Return full response + first item sample for inspection
        data = body.get("data", [])
        sample = data[0] if isinstance(data, list) and data else data
        return {
            "status_code":   resp.status_code,
            "total_items":   len(data) if isinstance(data, list) else "N/A",
            "sample_keys":   list(sample.keys()) if isinstance(sample, dict) else None,
            "sample_item":   sample,
            "full_response": body if resp.status_code != 200 else {"status": body.get("status"), "data_count": len(data) if isinstance(data, list) else 1},
        }
    except Exception as e:
...
async def get_recent_logs():
    """Return recent system activity for frontend debug panel."""
    import os
    logs = []
    # Read from Render log if available, else return structured status
    from data.upstox_market import _price_store, _instruments_loaded, _instruments_cache, is_ws_connected
    from api.upstox_auth import get_upstox_token
    token = await get_upstox_token()
    return {
        "upstox_token":       "SET" if token else "MISSING",
        "data_connected":     is_ws_connected(),
        "prices_cached":      dict(_price_store),
