"""Configuration — v3 (Professional Upgrade)"""
...
class Settings(BaseSettings):
    ...
    MAX_LOSS_STREAK_DAY_STOP: int = 3   # stop entire day after 3 losses
    COOLDOWN_MINUTES: int   = 20
    MIN_SCORE:        int   = 5

    # Adaptive position sizing
    RISK_SCORE_LOW:        float = 1.0   # score 5    → 1.0%
    RISK_SCORE_MID:        float = 1.5   # score 6-7  → 1.5%
    RISK_SCORE_HIGH:       float = 2.0   # score 8+   → 2.0%
    RISK_HIGH_ATR_MULT:    float = 0.7   # high ATR   → reduce 30%
    RISK_LOSS_STREAK_MULT: float = 0.5   # 2+ losses  → cut 50%
